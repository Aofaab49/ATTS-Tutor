...
const API_KEY = 'AIzaSyAbvQhYTtoNkf4qlyxcT-thDSOBewDg-9I';
...
function showError(error) {
    ...
    const message = error.message || (error.code ? errorMessages[error.code] : error);
    ...
    try {
        ...
        for (let i = 0; i < batchCount; i++) {
            ...
            try {
                ...
                // ลดการหน่วงเวลาลง หรือเอาออกถ้าไม่จำเป็น เพราะมี retry logic แล้ว
                if (i < batchCount - 1) {
        ...
        // แสดงข้อสอบ
        const formattedExam = formatExam(allQuestions);
...
เฉลย: [ก/ข/ค/ง]`;
                ...
                try {
                    ...
                    if (errorData.error) {
                        if (errorData.error.code === 429 || errorData.error.status === 'RESOURCE_EXHAUSTED') {
                            errorMessage = 'โควต้า API เต็มหรือยังไม่ได้เปิดใช้งาน (Quota Exceeded). กรุณาตรวจสอบ Billing หรือสร้าง Key ใหม่';
            ...
            // ล้างรูปแบบ Markdown ที่อาจติดมา
            generatedText = generatedText.replace(/\*\*/g, '').replace(/\*/g, '');

            // แยกข้อสอบโดยใช้ Regex ที่ยืดหยุ่นขึ้น
            let questions = generatedText.split(/\n(?=\d+\.)/).filter(q => q.trim());

            // กรองเฉพาะส่วนที่เป็นข้อสอบจริงๆ (ต้องขึ้นต้นด้วยตัวเลข)
            questions = questions.filter(q => q.match(/^\d+\./));

            // ถ้าแยกแล้วได้จำนวนไม่ตรง ลองแยกด้วยวิธีอื่น
            if (questions.length !== batchSize) {
                // ลองแยกด้วย \n\n
                const altQuestions = generatedText.split(/\n\n(?=\d+\.)/).filter(q => q.trim());
                if (altQuestions.length === batchSize) {
                    questions.length = 0;
            ...
            if (!validateBatchFormat(questions, batchSize)) {
                console.error('Validation failed for batch:', questions);
                throw new Error(`รูปแบบข้อสอบไม่ถูกต้อง (ได้ ${questions.length}/${batchSize} ข้อ)`);
            }

            // ตรวจสอบเนื้อหาว่าตรงกับวิชาหรือไม่
            if (!validateExamContent(questions.join('\n'), subject)) {
                throw new Error('เนื้อหาข้อสอบไม่ตรงกับวิชาที่ระบุ');
            }
            ...
            // ตรวจสอบว่าเป็น Error 429 (Too Many Requests) หรือไม่
            const isRateLimit = error.message.includes('429');

            // ถ้าเป็น 429 ให้รอนานขึ้น (เริ่มที่ 10 วินาที) ถ้าไม่ใช่รอแบบปกติ (เริ่มที่ 2 วินาที)
...
function validateBatchFormat(questions, expectedCount) {
    ...
    return questions.every(question => {
        ...
        // ตรวจสอบบรรทัดแรกว่าเป็นเลขข้อหรือไม่ (ยืดหยุ่นขึ้น)
        const hasQuestion = lines[0]?.match(/^\s*\d+\./);
        // ตรวจสอบตัวเลือก (ก-ง)
        const options = lines.filter(line => /^\s*[ก-ง]\./.test(line.trim()));
        // ตรวจสอบเฉลย
...
function formatExam(examData) {
    let questions;
    if (Array.isArray(examData)) {
        questions = examData;
    } else {
        questions = examData.split(/(?=\d+\.)/).filter(q => q.trim());
    }
    correctAnswers = [];
    ...
    return questions.map((question, index) => {
        ...
        // แยกตัวเลือก (รองรับเว้นวรรคข้างหน้า)
        const options = lines.filter(line => /^\s*[ก-ง]\./.test(line.trim()));
...
function showAnswers() {
    ...
    correctAnswers.forEach((correctAnswer, index) => {
        ...
        options.forEach(option => {
            ...
            const existingMarker = option.querySelector('.answer-badge');
            if (existingMarker) existingMarker.remove();

            if (optionValue === correctAnswer) {
                option.classList.add('correct');
                option.insertAdjacentHTML('beforeend',
                    '<span class="answer-badge correct">✓ เฉลย</span>');
            } else if (option === selectedOption) {
                if (optionValue !== correctAnswer) {
                    option.classList.add('incorrect');
                    option.insertAdjacentHTML('beforeend',
                        '<span class="answer-badge incorrect">✗</span>');
                }
            }
        });
    });
...
function resetExam() {
    document.querySelectorAll('.option-item').forEach(item => {
        ...
        const marker = item.querySelector('.answer-badge');
...
function validateExamContent(content, subject) {
    ...
    // ถ้าไม่มีคีย์เวิร์ดสำหรับวิชานั้นๆ ให้ผ่านไปเลย
    if (!subjectKeywords || subjectKeywords.length === 0 || subjectKeywords[0] === subject) {
        return true;
    }
    ...
    // ยอมรับถ้ามีข้อสอบอย่างน้อย 30% ที่ตรงกับคีย์เวิร์ด (เพื่อป้องกัน False Positive)
    const threshold = Math.ceil(questions.length * 0.3);
    const isValid = matchCount >= threshold;

    if (!isValid) {
        console.warn(`Content validation warning: Only ${matchCount}/${questions.length} questions matched keywords for ${subject}`);
    }

    // คืนค่า true เสมอเพื่อไม่ให้ขัดขวางการทำงาน แต่ log warning ไว้
    // (เปลี่ยนเป็น return isValid; ถ้าต้องการให้เข้มงวด)
    return true;
}
...
function getSubjectKeywords(subject) {
    ...
    const keywords = {
        'คณิตศาสตร์': ['คำนวณ', 'จำนวน', 'บวก', 'ลบ', 'คูณ', 'หาร', 'เรขาคณิต', 'สมการ', 'พื้นที่', 'รูป', 'มุม', 'เส้น', 'ร้อยละ', 'อัตราส่วน'],
        'ภาษาไทย': ['คำ', 'ประโยค', 'ความหมาย', 'อ่าน', 'เขียน', 'วรรณคดี', 'ภาษา', 'สะกด', 'วรรณยุกต์', 'มาตรา'],
        'ภาษาอังกฤษ': ['vocabulary', 'grammar', 'reading', 'writing', 'speaking', 'english', 'sentence', 'word', 'verb', 'noun', 'tense'],
        'วิทยาศาสตร์': ['ทดลอง', 'ธรรมชาติ', 'สิ่งมีชีวิต', 'พลังงาน', 'สสาร', 'แรง', 'สาร', 'ดิน', 'น้ำ', 'อากาศ', 'ดาว', 'เซลล์']
    };
